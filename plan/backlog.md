...
| T-005 | P0 | Build LLM generation endpoint with schema validation + retry | Team | Done | Requests JSON output with validation + retry; uses `PlanGenerationJob` for idempotency (no duplicate concurrent jobs per plan) |

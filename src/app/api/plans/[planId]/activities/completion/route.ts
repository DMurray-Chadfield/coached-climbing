...
export async function PATCH(
  ...
  try {
    ...
    if (!parsed.success) {
      return jsonError(400, "INVALID_PAYLOAD", "Invalid activity completion payload.", parsed.error.flatten());
    }
    ...
    const completion = await toggleActivityCompletion({
      userId,
    ...
    if (error instanceof Error && ["INVALID_PLAN_JSON", "INVALID_SESSION", "INVALID_ACTIVITY"].includes(error.message)) {
      return jsonError(400, "INVALID_PLAN_REFERENCE", "Invalid week/session/activity for this plan version.");
    }

    return jsonError(500, "INTERNAL_ERROR", "Unable to update activity completion.");
  }
}

...
export async function PATCH(
  ...
  try {
    ...
    if (!parsed.success) {
      return jsonError(400, "INVALID_PAYLOAD", "Invalid session note payload.", parsed.error.flatten());
    }
    ...
    const notes = await setSessionNote({
      userId,
    ...
    return jsonError(500, "INTERNAL_ERROR", "Unable to update session note.");
  }
}
